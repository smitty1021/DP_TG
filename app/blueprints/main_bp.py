...
def get_time_in_trade_minutes(trade):
    ...
    try:
        ...
        # Get first entry
        first_entry = trade.entries.order_by('entry_time').first()
        # Get last exit
        last_exit = trade.exits.order_by('exit_time desc').first()
...
@login_required
def dashboard_data():
    """API endpoint to serve dashboard data as JSON - much safer than template embedding"""
    try:
        # Get all trades for the current user
        trades = Trade.query.filter_by(user_id=current_user.id).order_by(Trade.trade_date.asc()).all()
        ...
        def get_time_in_trade_minutes(trade):
            ...
            try:
                ...
                # Get first entry - use proper SQLAlchemy syntax
                from sqlalchemy import asc, desc
                first_entry = trade.entries.order_by(asc('entry_time')).first()
                # Get last exit - use proper SQLAlchemy syntax
                last_exit = trade.exits.order_by(desc('exit_time')).first()
                ...
                # Don't log the error anymore since it's working, just return None
                return None
                ...
                for trade in trades:
                    try:
                        trade_dict = {
                            ...
                            'exit_price': float(trade.average_exit_price) if trade.average_exit_price else 0,
                            'pnl': float(trade.pnl) if trade.pnl else 0,
                            'time_in_trade': None,  # Simplified for now
                            'entry_time': None,  # Simplified for now
                            'exit_time': None,  # Simplified for now
                            'how_closed': trade.how_closed
                        }
